...
class MLKGCL(KnowledgeRecommender):
    input_type = InputType.PAIRWISE
    def __init__(self, config, dataset):
        super(MLKGCL, self).__init__(config, dataset)
        ...
        # cts_lamda
        self.lamda_user_item = config['cts_lamda']
        self.lamda_user = config['cts_lamda']
        self.lamda_entity = config['cts_lamda']

        # Component Settings
        self.user_level = config['user_level']
        self.entity_level = config['entity_level']
        self.user_item_level = config['user_item_level']
        self.with_projectHead = config['with_projectHead']
        ...
        total_emb = 0
        for ele in range(0, len(config['layers'])):
            if ele == 0:
                total_emb = config['layers'][ele] * 2
            else:
                total_emb = total_emb + config['layers'][ele]
        inner_size = total_emb
    ...
    def calculate_loss(self, interaction):
        ...
        # Joint Learning loss
        loss = mf_loss + self.reg_weight * reg_loss + all_level_loss
        return loss
